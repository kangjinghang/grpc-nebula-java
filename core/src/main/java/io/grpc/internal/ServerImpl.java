...
public final class ServerImpl extends Server implements InternalInstrumented<ServerStats> {
  ...
  // 服务注册
  private ProviderServiceRegistry providerRegistry = ProviderServiceRegistryFactory.getRegistry();
  ...
  public ServerImpl shutdown() {
    ...
    if (shutdownTransportServer) {
      ...
      //----begin----服务关闭时，自动注销zk中的Provider信息----

      List<Map<String,Object>> params = createServerParams();
      providerRegistry.unRegister(params);

      //----end----服务关闭时，自动注销zk中的Provider信息----
    }
